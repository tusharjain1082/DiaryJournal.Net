        ...
        private void TvEntries_DragDrop(object? sender, DragEventArgs e)
        {
            ...
            if (!draggedNode.Equals(targetNode) && !ContainsNode(draggedNode, targetNode))
            {
                ...
                if (e.Effect == DragDropEffects.Move)
                {
                    ...
                    {
                        if (!OpenFileSystemDB.setNodeParent(ctx1, 
                            draggedNodeEntry.chapter.guid, ref updatedNode, targetNodeEntry.chapter.guid))
                            return;
                    }
                    else
                    {
                        if (!SingleFileDB.setNodeParent(ctx0, draggedNodeEntry.chapter.guid, ref updatedNode, targetNodeEntry.chapter.guid))
                            return; // error, could not operate

                    }
                    draggedNode.Remove();
                    draggedNode.Tag = updatedNode;
                    String path = String.Format(@"{0}", updatedNode.chapter.guid);
                    draggedNode.Name = path;
...
//                var res = MessageBox.Show(this, "you cannot directly close this form. please click on exit button to close the form.", "error",
  //                      MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            if (!ctx0.isDBOpen() && !ctx1.isDBOpen())
                return;
            ...
            if (cfg.radCfgUseOpenFileSystemDB)
            {
                node = OpenFileSystemDB.findLoadNode(ctx1, guid, ref rtf, false);
            }
            else
            {
                node = SingleFileDB.findLoadNode(ctx0, guid, ref rtf, false);
            }
            if (node == null)
                return;
        ...
        public void viewSelectedTrashCanEntry()
        {
            ...
            String rtf = "";
            if (cfg.radCfgUseOpenFileSystemDB)
                node = OpenFileSystemDB.findLoadNode(ctx1, guid, ref rtf, true);
            else
                node = SingleFileDB.findLoadNode(ctx0, guid, ref rtf, true);
        ...
        public void setupNewEntry(DateTime dateTime, myNode? parent = null)
        {
            ...
            {
                // init calendar nodes
                entryMethods.initCalenderNodes_OpenFileSystemDB(ctx1, dateTime.Year, dateTime.Month, out yearNode, out monthNode);
                ...
                // create new node
                newNode = OpenFileSystemDB.newNode(ref ctx1, NodeType.EntryNode, dateTime, parentChapter.guid, true, "");
                if (newNode == null)
                    return;

                // now load up all parents and root ancestor of this node.
                List<myNode> lineageChain = OpenFileSystemDB.findBottomToRootNodesRecursive(ctx1, newNode.chapter.guid);
                ...
                totalNodes = OpenFileSystemDB.NodesCount(ctx1);
            }
            else
            {
                // init calendar nodes
                entryMethods.initCalenderNodes_SingleFileDB(ctx0, dateTime.Year, dateTime.Month, out yearNode, out monthNode);
                ...
                newNode = SingleFileDB.newNode(ctx0, NodeType.EntryNode, ref chapterData, dateTime, "", parentChapter, true);
                if (newNode == null)
                    return;

                // now load up all parents and root ancestor of this node.
                List<myNode> lineageChain = SingleFileDB.findBottomToRootNodesRecursive(ctx0, newNode.chapter.guid);
                if (lineageChain.Count <= 0)
                    return;
                ...
                totalNodes = SingleFileDB.ChaptersCount(ctx0);
            }
            ...
            tsslblTotalEntries.Text = totalNodes.ToString();
        ...
        public void __createNewDB_SingleFileDB()
        {
            ...
            // todo
            if (!SingleFileDB.CreateLoadDB(sfdDB.FileName, ref ctx0, true))
            {
                reset();
        ...
        public void __createNewDB_OpenFileSystemDB()
        {
            ...
            // todo
            if (!OpenFileSystemDB.createLoadDB(browseFolder.SelectedPath, input, ref ctx1, true, true))
            {
                reset();
                this.Invoke(showMessageBox, "errror creating/loading db", "error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            ...
            // setup ui
            txtDBFile.Text = ctx1.dbBasePath;
        }
        ...
        public void importEntries(String path, EntryType entryType)
        {
            ...
            for (long i = 0; i < totalFiles; i++)
            {
                ...
                if (cfg.radCfgUseOpenFileSystemDB)
                    OpenFileSystemDB.createNode(ctx1, ref node, rtf);
                else
                    SingleFileDB.createNode(ctx0, ref node, rtf);
        ...
        public void importTheJournalRtfEntries(String path)
        {
            ...
            foreach (string file in files)
            {
                ...
                // get rtf and update
                // richtextbox cleans and fixes the rtf and makes it valid.
                ...
                if (cfg.radCfgUseOpenFileSystemDB)
                    entryMethods.initCalenderNodes_OpenFileSystemDB(ctx1, chapter.chapterDateTime.Year, 
                        chapter.chapterDateTime.Month, out yearNode, out monthNode);
                else
                    entryMethods.initCalenderNodes_SingleFileDB(ctx0, chapter.chapterDateTime.Year,
                        chapter.chapterDateTime.Month, out yearNode, out monthNode);
                ...
                {
                    // import direct
                    if (!OpenFileSystemDB.createNode(ctx1, ref node, rtf))
                        continue;
                }
                else
                {
                    // import direct
                    if (!SingleFileDB.createNode(ctx0, ref node, rtf))
                        continue;
                }
        ...
        public void SingleFileDBToOpenFSDB(String path, String dbName, EntryType entryType)
        {
            // first get the entry type and formats
            String ext = "";
            String extComplete = "";
            ...
            long ChaptersCount = SingleFileDB.ChaptersCount(ctx0);
            long exportIndex = 0;
            if (ChaptersCount <= 0)
                return;
            ...
            OpenFileSystemDB.createLoadDB(path, dbName, ref destctx, true);

            // collect all nodes
            List<myNode> nodes = SingleFileDB.findAllNodes(ctx0, true, false);
        ...
        {
            if (cfg.radCfgUseOpenFileSystemDB)
                __exportEntries_OpenFileSystemDB(path, entryType);
            else
                __exportEntries_SingleFileDB(path, entryType);

        }


        public void __exportEntries_SingleFileDB(String path, EntryType entryType)
        {
            // first get the entry type and formats
            String ext = "";
            String extComplete = "";
            ...
            foreach (myNode node in nodes)
            {
                if (!entryMethods.exportEntry_SingleFileDB(ctx0, node.chapter.guid, path, exportIndex, entryType))
                    continue;
        ...
        public void __exportEntries_OpenFileSystemDB(String path, EntryType entryType)
        {
            // first get the entry type and formats
            String ext = "";
            String extComplete = "";
            ...
            // first get the total number of chapters which exist in db
            long nodesCount = OpenFileSystemDB.NodesCount(ctx1);
            long exportIndex = 0;
            ...
            // collect all nodes
            List<myNode> nodes = OpenFileSystemDB.findAllNodes(ctx1, true, false);

            foreach (myNode node in nodes)
            {
                if (!entryMethods.exportEntry_OpenFileSystemDB(ctx1, node.chapter.guid, path, exportIndex, entryType))
                    continue;
        ...
        {
            if(cfg.radCfgUseOpenFileSystemDB)
                __loadEntries_OpenFileSystemDB();
            else
                __loadEntries_SingleFileDB();
        }
     
        public void __loadEntries_SingleFileDB()
        {
            if (!ctx0.isDBOpen())
                return;
            ...
            // upgrade and past bug fix
            foreach (myNode node in nodes)
            {
                // now load up all parents and root ancestor of this node.
                List<myNode> lineageChain = SingleFileDB.findBottomToRootNodesRecursive(ctx0, node.chapter.guid);
                ...
                // update
                filesDone += 1;
                this.Invoke(updateProgressStatus, filesDone, ChaptersCount);
            ...
            // update
            totalNodes = filesDone;
            this.Invoke(updateProgressStatus, 0, 0);
            this.Invoke(updateTotalEntriesStatus, totalNodes);
            this.Invoke(treeViewEndUpdate, tvEntries);
            this.Invoke(toggleForm, true);
            this.Invoke(gotoTodaysEntry);
        }
        public void __loadEntries_OpenFileSystemDB()
        {
            if (!ctx1.isDBOpen())
                return;
            ...
            { 
                // now load up all parents and root ancestor of this node.
                List<myNode> lineageChain = OpenFileSystemDB.findBottomToRootNodesRecursive(ctx1, node.chapter.guid);
        ...
        public void loadDB(String path)
        {
            ...
            if (cfg.radCfgUseOpenFileSystemDB)
            {
                if (!OpenFileSystemDB.LoadDB(ref ctx1, cfg, path))
                {
                    reset();
            ...
            { 
                if (!SingleFileDB.CreateLoadDB(path, ref ctx0, false))
                {
                    reset();
                    this.Invoke(showMessageBox, "errror creating/loading db", "error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
        ...
        public void __saveEntry()
        {
            ...
            myNode idNode = (myNode)tvEntries.SelectedNode.Tag;
            if (idNode == null)
                return;
            ...
            if (cfg.radCfgUseOpenFileSystemDB)
            {
                ...
                node = OpenFileSystemDB.findLoadNode(ctx1, idNode.chapter.guid, ref rtf, false);
                if (node == null)
                    return;

                // finally save/update
                OpenFileSystemDB.updateNode(ctx1, ref node, rtbEntry.Rtf, true);
            }
            else
            {
                // first load the actual node
                node = SingleFileDB.findLoadNode(ctx0, idNode.chapter.guid, ref rtf, false);
                if (node == null)
                    return;

                // finally save/update
                SingleFileDB.updateNode(ctx0, ref node, rtbEntry.Rtf, true);
            }
        ...
        public void setupEntryTitle(String title)
        {
            ...
            if (cfg.radCfgUseOpenFileSystemDB)
            {
                if (!OpenFileSystemDB.updateNodeTitle(ctx1, node.chapter.guid, title))
                    return;
            }
            else
            {
                if (!SingleFileDB.updateNodeTitle(ctx0, node.chapter.guid, title))
                    return;
            }
            // resetup tree node
            String entryName = "";
...
//            tsbuttonSave.Checked = true;
        ...
        public void __gotoEntry(DateTime dateTime)
        {
            ...
            if (cfg.radCfgUseOpenFileSystemDB)
            {
                List<myNode> list = OpenFileSystemDB.findNodesByDate(ctx1, NodeType.AnyOrAll, dateTime.Year, dateTime.Month, dateTime.Day);
                if (list.Count <= 0)
                    return;
            ...
            {
                List<myNode> list = SingleFileDB.findNodesByDate(ctx0, NodeType.AnyOrAll, dateTime.Year, dateTime.Month, dateTime.Day);
                if (list.Count <= 0)
                    return;
        ...
        public void autoCreateLoadDefaultDB(bool overwrite)
        {
            ...
            if (cfg.radCfgUseOpenFileSystemDB)
            {
                ...
                if (!OpenFileSystemDB.autoLoadCreateDefaultDB(ref ctx1, cfg, overwrite))
                {
            ...
            {
                // direct auto create/load default db
                if (!SingleFileDB.autoLoadCreateDefaultDB(ctx0, cfg, overwrite))
                {
                    MessageBox.Show("error creating/loading the default db. aborted.", "error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return;
                }
        ...
        public void findTotalEntriesInDB()
        {
            ...
            MessageBox.Show("total number of " + totalNodes + " validated/valid entries actually exist in db.", "status", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        public void deleteEntries()
        {
            ...
            foreach (TreeNode rootNode in rootNodes)
            {
                ...
                while (queue.Count > 0)
                {
                    ...
                    if (cfg.radCfgUseOpenFileSystemDB)
                        OpenFileSystemDB.DeleteOrPurgeRecursive(ctx1, nodeEntry.chapter.guid, true, false);
                    else
                        SingleFileDB.DeleteOrPurgeRecursive(ctx0, nodeEntry.chapter.guid, true, false);
                }
            }
        ...
        public void refreshTrashcan()
        {
            ...
            if (cfg.radCfgUseOpenFileSystemDB)
                nodes = OpenFileSystemDB.findDeletedNodes(ctx1);
            else
                nodes = SingleFileDB.findDeletedNodes(ctx0);
        ...
        public void purgeTrashCan()
        {
            ...
            foreach (ListViewItem listViewItem in lvTrashCan.CheckedItems)
            {
                Guid guid = (Guid)listViewItem.Tag;
                if (cfg.radCfgUseOpenFileSystemDB)
                    OpenFileSystemDB.DeleteOrPurgeRecursive(ctx1, guid, false, true);
                else
                    SingleFileDB.DeleteOrPurgeRecursive(ctx0, guid, false, true);
            }
        ...
        public void restoreTrashCan()
        {
            ...
            foreach (ListViewItem listViewItem in lvTrashCan.CheckedItems)
            {
                Guid guid = (Guid)listViewItem.Tag;
                if (cfg.radCfgUseOpenFileSystemDB)
                    OpenFileSystemDB.DeleteOrPurgeRecursive(ctx1, guid, false, false);
                else
                    SingleFileDB.DeleteOrPurgeRecursive(ctx0, guid, false, false);
        ...
        public void highlightEntry(TreeNode node)
        {
            ...
            entryMethods.setEntryHighlightFont(ctx0, ctx1, cfg, identifier.chapter, fontDialog.Color, fontDialog.Font);
        ...
        public void highlightTreeViewNode(TreeNode node, Font font, Color color)
        {
            ...
            node.NodeFont = font;
            entryMethods.setEntryHighlightFont(ctx0, ctx1, cfg, identifier.chapter, color, font);
            loadNodeHighlight(node);
        ...
        public void highlightEntryBackColor(TreeNode node)
        {
            ...
            entryMethods.setEntryHighlightBackColor(ctx0, ctx1, cfg, identifier.chapter, colorDialog.Color);
            loadNodeHighlight(node);
        }
        ...
        public bool __processSearch()
        {
            ...
            if (cfg.radCfgUseOpenFileSystemDB)
            {
                return journalSearchFramework.searchEntries_OpenFileSystemDB(ctx1, lvSearch, tsSearchProgressBar,
            ...
            {
                return journalSearchFramework.searchEntries_SingleFileDB(ctx0, lvSearch, tsSearchProgressBar,
                    dtpickerSearchFrom.Value, dtPickerSearchFromTime.Value, dtpickerSearchThrough.Value, dtpickerSearchThroughTime.Value,
                    chkSearchUseDateRange.Checked, rtbSearchPattern.Text, rtbSearchReplace.Text, chkSearchAll.Checked,
                    chkSearchTrashCan.Checked, chkSearchMatchCase.Checked, chkSearchMatchWholeWord.Checked,
                    chkSearchReplace.Checked, chkSearchMultiline.Checked, chkSearchExplicitCaptures.Checked,
        ...
        public void restoreSearchedList()
        {
            ...
            foreach (ListViewItem listViewItem in lvSearch.CheckedItems)
            {
                ...
                if (cfg.radCfgUseOpenFileSystemDB)
                {
                    myNode? node = OpenFileSystemDB.findLoadNode(ctx1, guid, ref rtf, false);
                    if (node == null)
                ...
                {
                    myNode? node = SingleFileDB.findLoadNode(ctx0, guid, ref rtf, false);
                    if (node == null)
                        return; // node not found
        ...
        public void deleteSearchedList()
        {
            ...
            foreach (ListViewItem listViewItem in lvSearch.CheckedItems)
            {
                ...
                if (cfg.radCfgUseOpenFileSystemDB)
                {
                    ...
                    if (!node.chapter.IsDeleted)
                        OpenFileSystemDB.DeleteOrPurgeRecursive(ctx1, guid, true, false);
                }
                else
                {
                    myNode? node = SingleFileDB.findLoadNode(ctx0, guid, ref rtf, false);
                    ...
                    if (!node.chapter.IsDeleted)
                        SingleFileDB.DeleteOrPurgeRecursive(ctx0, guid, true, false);
                }
                listViewItem.Checked = false;
                listViewItem.Selected = true;
            }
        ...
        public void exportSelectedEntry(EntryType entryType)
        {
            ...
            // finally export
            if (cfg.radCfgUseOpenFileSystemDB)
                entryMethods.exportEntry_OpenFileSystemDB(ctx1, node.chapter.guid, browseFolder.SelectedPath, 0, entryType);
            else
                entryMethods.exportEntry_SingleFileDB(ctx0, node.chapter.guid, browseFolder.SelectedPath, 0, entryType);
        }
        ...
        public void importNewEntry(EntryType InputEntryType)
        {
            ...
            if (cfg.radCfgUseOpenFileSystemDB)
            {
                entryMethods.initCalenderNodes_OpenFileSystemDB(ctx1, node.chapter.chapterDateTime.Year, node.chapter.chapterDateTime.Month, out yearNode, out monthNode);
            }
            else
            {
                entryMethods.initCalenderNodes_SingleFileDB(ctx0, node.chapter.chapterDateTime.Year, node.chapter.chapterDateTime.Month,
                    out yearNode, out monthNode);
            }
        ...
        private void bgWorkerSingleDBToFSDB_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            SingleFileDBToOpenFSDB(browseFolder.SelectedPath, dbName, EntryType.Rtf);
        }
