    ...
    public static class SingleFileDB
    {
        ...
        public static bool createDb(String file)
        {
            try
            {
                if (File.Exists(file))
                    File.Delete(file);
                    //myCommonMethods1.SecureEraseFile(file, 1, true);
                //File.Delete(file);
            }
            catch (Exception)
            {
        ...
        */
        ...
        public static bool NodeBackup(SingleFileDBContext ctx, Int64 id)
        {

            String rtf = "";
            myNode? node = findLoadNode(ctx, id, ref rtf, true);
            if (node == null)
                return false;

            // no custom file name given, so auto generate filename based on database node/entry attributes.
            String entryName = "";
            String entryFile = entryMethods.getFormattedPathFileName("", node.chapter.Id, node.chapter.parentId, node.chapter.Title,
                node.chapter.chapterDateTime, 0, EntryType.Rtf, out entryName);
            String entryConfigFile = entryMethods.getFormattedPathFileName("", node.chapter.Id, node.chapter.parentId, node.chapter.Title,
                node.chapter.chapterDateTime, 0, EntryType.Cfg, out entryName);

            // create entry's config string
            String entryConfigString = cfgEntry.toCfg(ref node.chapter);

            commonMethods.commonMethods.autoCreateBackupEntryFileSFDB(entryFile, entryConfigFile, rtf, entryConfigString, DateTime.Now);
            return true;
        }
        ...
        public static bool updateNode(SingleFileDBContext ctx, ref myNode node, String rtf = "", bool storeData = false, bool updateModificationDate = true,
             bool checkpoint = true, bool backupOldEntryFirst = false)
        {
            // first backup the entire node if required
            if (backupOldEntryFirst)
                NodeBackup(ctx, node.chapter.Id);
