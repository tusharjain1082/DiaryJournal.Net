...
namespace DiaryJournal.Net
{
    ...
    public partial class FrmJournal : Form
    {
        ...
        private void TvEntries_DragDrop(object? sender, DragEventArgs e)
        {
            ...
            if (targetNode.chapter.specialNodeType == SpecialNodeType.SystemNode)
            {
                // system node
            ...
            if (draggedNode.chapter.specialNodeType == SpecialNodeType.SystemNode)
            {
                ...
                //}
                // we cannot move/drag/change system node
                return;
            }
...
//                var res = MessageBox.Show(this, "you cannot directly close this form. please click on exit button to close the form.", "error",
  //                      MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public void setupNewEntry(DateTime dateTime, UInt32 parentId = 0, NodeType nodeType = NodeType.EntryNode,
            String title = "")
        {
            if (!cfg.ctx0.isDBOpen() && !cfg.ctx1.isDBOpen())
                return;
            ...
            if (cfg.radCfgUseOpenFileSystemDB)
            {
                ...
                lineageChain = OpenFileSystemDB.findBottomToRootNodesRecursive(cfg.ctx1, node.chapter.nodeID);
                cfg.totalNodes = OpenFileSystemDB.NodesCount(cfg.ctx1);
            }
            else
            {
                // now load up all parents and root ancestor of this node.
                lineageChain = SingleFileDB.findBottomToRootNodesRecursive(cfg.ctx0, node.chapter.nodeID);
        ...
        */
        ...
        public void __importTheJournalRtfEntries(String path)
        {
            ...
            List<myNode> allNodes = entryMethods.DBFindAllChildrenRecursive(cfg, setNode.chapter.nodeID, true, false);
            ...
            foreach (FileInfo file in files)
            {
                ...
                newNode = entryMethods.DBNewNode(ref cfg,
                    ...
                    ref newNode, chapter.chapterDateTime, chapter.parentNodeID, true, rtf,
                    true, false);
...
//            tsbuttonSave.Checked = true;
        ...
        public void deleteEntries()
        {
            ...
            List<myNode> highestNodes = __getHighestCheckedTreeViewItemsDBNodes();
            List<myNode> allNodes = entryMethods.DBFindAllNodesTreeSequence(cfg, true, false);
            entryMethods.DBDeleteOrPurgeListRecursive(cfg, ref allNodes, ref highestNodes, true, false);
