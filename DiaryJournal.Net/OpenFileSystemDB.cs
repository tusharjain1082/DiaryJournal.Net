...
namespace DiaryJournal.Net
{
    ...
    public static class OpenFileSystemDB
    {
        ...
        // find all root nodes
        public static List<myNode> findRootNodes(OpenFSDBContext ctx, bool sort = true, bool descending = false)
        {
            List<myNode> nodes = new List<myNode>();
            foreach (myNode listNode in findAllNodes(ctx, true, false))
            {
                myNode node = listNode;
                if (node.chapter.parentGuid == Guid.Empty)
                    nodes.Add(node);
            }
        ...
        public static List<myNode> findFirstLevelChildren(OpenFSDBContext ctx, Guid parentGuid, bool sort = true, bool descending = false)
        {
            List<myNode> nodes = new List<myNode>();
