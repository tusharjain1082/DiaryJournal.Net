...
namespace DiaryJournal.Net
{
    ...
    public static class OpenFileSystemDB
    {
        ...
        public static bool LoadDB(ref OpenFSDBContext ctx, myConfig cfg, String dbPath)
        {
            if (dbPath == "")
                return false;
        ...
        public static List<myNode> findBottomToRootNodesRecursive(OpenFSDBContext ctx, Guid guid)
        {
            List<myNode> nodes = new List<myNode>();

            // first find the current node
            String rtf = "";
            //myNode node = findLoadNode(ctx, guid, Guid.Empty, false, entryType, ref rtf, false);
            ...
            while (true)
            {
                ...
                //node = findLoadNode(ctx, Guid.Empty, node.chapter.parentGuid, true, entryType, ref rtf, false);
        ...
        public static bool DeleteOrPurgeRecursive(OpenFSDBContext ctx, Guid guid, bool mark = true, bool purge = false)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                // finally update the node
                updateNode(ctx, ref currentNode, null);

                // purge the marked nodes if demanded
                if (purge)
                    purgeNode(ctx, ref currentNode);
            }
            return true;
        }
