...

{
    public static class journalSearchFramework
    {
            ...
            bool searchReplaceTitle)
        {
            ...
            List<myNode>? nodes = entryMethods.DBFindAllNodes(cfg, false, false);
            nodes = entryMethods.findAllNodesTreeSequence(ref nodes, true, false);
            long nodesCount = nodes.LongCount();
            if (nodesCount <= 0)
                return false;
            ...
            foreach (myNode listNode in nodes)
            {
                ...
                if (cfg.radCfgUseOpenFileSystemDB)
                    rtf = OpenFileSystemDB.loadNodeData(cfg.ctx1, node.chapter.Id);
                else
