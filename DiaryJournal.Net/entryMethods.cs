...
namespace DiaryJournal.Net
{
    public static class entryMethods
    {
        ...
        public static bool exportEntry_SingleFileDB(SingleFileDBContext ctx, Guid guid, String path,
            long exportIndex, EntryType entryType)
        {
            if (guid == Guid.Empty)
                return false;
            ...
            String rtf = "";
            myNode? node = SingleFileDB.findLoadNode(ctx, guid, ref rtf, true);
            if (node == null)
                return false;
            ...
            path = entryMethods.getFormattedPathFileName(path, node.chapter.guid, node.chapter.parentGuid, node.chapter.Title,
                node.chapter.chapterDateTime, exportIndex, entryType, out entryName);
        ...
        public static bool exportEntry_OpenFileSystemDB(OpenFSDBContext ctx, Guid guid, String path,
            long exportIndex, EntryType OutputEntryType)
        {
            ...
            // 1st load chapter's data blob
            String rtf = "";
            myNode? node = OpenFileSystemDB.findLoadNode(ctx, guid, ref rtf, true);
        ...
        public static bool deleteChapterEntry(SingleFileDBContext ctx, Chapter? identifier, bool mark)
        {
            if (identifier == null)
                return false;
            ...
            return SingleFileDB.DeleteOrPurgeRecursive(ctx, identifier.guid, true, false);
        }

        public static bool setEntryHighlightFont(SingleFileDBContext ctx0, OpenFSDBContext ctx1, 
            myConfig cfg, Chapter identifier, Color highlightFontColor, Font highlightFont)
        {
            ...
            if (cfg.radCfgUseOpenFileSystemDB)
                return OpenFileSystemDB.updateHLFont(ctx1, identifier);
            else
                return SingleFileDB.updateChapterHLFontByGuid(ctx0, identifier);
        }

        public static bool setEntryHighlightBackColor(SingleFileDBContext ctx0, OpenFSDBContext ctx1,
            myConfig cfg, Chapter identifier, Color highlightBackColor)
        {
            ...
            if (cfg.radCfgUseOpenFileSystemDB)
                return OpenFileSystemDB.updateHLBackColor(ctx1, identifier);
            else
                return SingleFileDB.updateChapterHLBackColorByGuid(ctx0, identifier);
        }

        public static bool initCalenderNodes_SingleFileDB(SingleFileDBContext ctx, int year, int month, out myNode? yearNodeOut, out myNode? monthNodeOut)
        {
            // now create Year Node direct in DB if it doesn't exists. else load it from db for linking
            ChapterData? yearChapterData = null;
            List<myNode> yearNodes = SingleFileDB.findNodesByNodeType(ctx, NodeType.YearNode, year, -1);
            ...
            if (yearNodes.Count() <= 0)
            {
                ...
                yearNode = SingleFileDB.newNode(ctx, NodeType.YearNode, ref yearChapterData, yearDateTime, "", null, true);
            }
            else
            {
            ...
            List<myNode> monthNodes = SingleFileDB.findNodesByNodeType(ctx, NodeType.MonthNode, year, month);
            ...
            if (monthNodes.Count() <= 0)
            {
                ...
                monthNode = SingleFileDB.newNode(ctx, NodeType.MonthNode, ref monthChapterData, monthDateTime, "", yearNode.chapter, true);
            }
            else
            {
        ...
        public static bool initCalenderNodes_OpenFileSystemDB(OpenFSDBContext ctx,
            int year, int month, out myNode? yearNodeOut, out myNode? monthNodeOut)
        {
            // now create Year Node direct in DB if it doesn't exists. else load it from db for linking
            List<myNode> yearNodes = OpenFileSystemDB.findNodesByDate(ctx, NodeType.YearNode, year, month, -1);
            myNode? yearNode = null;
            if (yearNodes.Count() <= 0)
            {
                // this year doesn't exists in db, so create it.
                DateTime yearDateTime = new DateTime(year, 1, 1, 0, 0, 0, 0);
                yearNode = OpenFileSystemDB.newNode(ref ctx, NodeType.YearNode, yearDateTime, default(Guid), true, "");
            }
            else
            {
                // this year node already exists, so directly load it to the object.
            ...
            // now create Month Node direct in DB if it doesn't exists. else load it from db for linking.
            // note Month node's parent must be the year node.
            List<myNode> monthNodes = OpenFileSystemDB.findNodesByDate(ctx, NodeType.MonthNode, year, month, -1);
            myNode? monthNode = null;
            if (monthNodes.Count() <= 0)
            {
                // this month in the current chapter's year doesn't exists in db, so create it.
                DateTime monthDateTime = new DateTime(year, month, 1, 0, 0, 0, 0);
                monthNode = OpenFileSystemDB.newNode(ref ctx, NodeType.MonthNode, monthDateTime, yearNode.chapter.guid, true, "");
            }
            else
            {
                // this month node in the current chapter's year already exists, so directly load it to the object.
