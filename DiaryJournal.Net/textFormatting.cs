...
namespace DiaryJournal.Net
{
    public class textFormatting
    {
        ...
        public bool formatCells(WpfRichTextBoxEx rtb)
        {
            ...
            Paragraph cellpara0 = null;
            Inline cellinline0 = null;
            if (cell.Blocks.Count > 0)
            {
                Block? cellblock0 = cell.Blocks.FirstBlock;
                if (cellblock0 is Paragraph)
                {
                    cellpara0 = (Paragraph)cellblock0;
                    if (cellpara0.Inlines.Count > 0)
                        cellinline0 = cellpara0.Inlines.First();
                }
            }
            */
            ...
            Paragraph cellpara0 = null;
            Inline cellinline0 = null;
            if (rtb.GetAllInlinesRecursive(cell.Blocks.FirstBlock, out inlines, 1))
            {
                cellpara0 = (Paragraph)cell.Blocks.FirstBlock;
                cellinline0 = inlines.FirstOrDefault();
            }
            ...
            form.cellAlignment = ((cellpara0 != null) ? cellpara0.TextAlignment : TextAlignment.Left);
            form.Italic = ((cellpara0 != null) ? rtb.WpfItalicToWinFormsItalic(cellpara0.FontStyle) : rtb.WpfItalicToWinFormsItalic(FontStyles.Normal));
            form.Bold = ((cellpara0 != null) ? rtb.WpfBoldToWinFormsBold(cellpara0.FontWeight) : rtb.WpfBoldToWinFormsBold(FontWeights.Normal));
            form.fontColor = ((cellinline0 != null) ? WpfRichTextBoxEx.WpfBrushToWinFormsColor((SolidColorBrush)cellinline0.Foreground) : System.Drawing.Color.Black);
            form.fontBackColor = ((cellinline0 != null) ? WpfRichTextBoxEx.WpfBrushToWinFormsColor((SolidColorBrush)cellinline0.Background) : System.Drawing.Color.White);
            form.font = ((cellpara0 != null) ? rtb.WpfFontToWinFormsFont(cellpara0.FontFamily) : rtb.WpfFontToWinFormsFont(new System.Windows.Media.FontFamily("Times New Roman")));
            form.fontName = form.font.Name;
            form.fontSize = ((cellpara0 != null) ? (int)cellpara0.FontSize : (int)cell.FontSize); 
        ...
        */
        ...
        public bool formatColumns(WpfRichTextBoxEx rtb, bool removeFormatting)
        {
            ...
            if (!removeFormatting)
            {
                // if user does not wants to remove all formatting, then we show formatting form and configure
                form.isEditingCells = true;
                form.cellAlignment = ((cellpara0 != null) ? cellpara0.TextAlignment : TextAlignment.Left);
                form.Italic = ((cellpara0 != null) ? rtb.WpfItalicToWinFormsItalic(cellpara0.FontStyle) : rtb.WpfItalicToWinFormsItalic(FontStyles.Normal));
                form.Bold = ((cellpara0 != null) ? rtb.WpfBoldToWinFormsBold(cellpara0.FontWeight) : rtb.WpfBoldToWinFormsBold(FontWeights.Normal));
        ...
        public bool formatInsertTable(WpfRichTextBoxEx rtb)
        {
            ...
            */
            ...
            Block? currentSelBlock = rtb.ObjectAtSelectionStart();
            ...
            foreach (TableRow row in table.RowGroups[0].Rows)
            {
                foreach (TableCell cell in row.Cells)
                {
                    ...
                    run.FontSize = form.fontSize;
                    run.FontFamily = new System.Windows.Media.FontFamily(form.fontName);
                    run.FontStyle = rtb.WinFormsItalicToWpfItalic(form.Italic);
                    run.FontWeight = rtb.WinFormsBoldToWpfBold(form.Bold);
                    run.Foreground = WpfRichTextBoxEx.WinFormsColorToWpfBrush(form.fontColor);
                    run.Background = WpfRichTextBoxEx.WinFormsColorToWpfBrush(form.fontBackColor);
                }
            }
