...

{
    public static class journalSearchFramework
    {
        ...
        public static bool searchEntries_SingleFileDB(SingleFileDBContext ctx, ListView lv, ToolStripProgressBar tsProgressBar,
            ...
            bool searchReplaceTitle)
        {
            ...
            long nodesCount = SingleFileDB.NodesCount(ctx);
            if (nodesCount <= 0)
                return false;
            ...
            rtb.SuspendLayout();
            rtb.BeginUpdate();
            List<myNode> nodes = SingleFileDB.findAllNodes(ctx, true, false);
            long nodesDone = 0;

            foreach (myNode listNode in nodes)
            {
                myNode node = listNode;
                String rtf = SingleFileDB.loadNodeData(ctx, node.chapter.guid);

                //myNode? node = OpenFileSystemDB.pathToNode(ctx, file, ref rtf, true);
                ...
                if (matchesFound)
                {
                    SingleFileDB.updateNode(ctx, ref node, rtb.Rtf, true);
        ...
        public static bool searchEntries_OpenFileSystemDB(OpenFSDBContext ctx, ListView lv, ToolStripProgressBar tsProgressBar,
            DateTime inputFrom, DateTime inputFromTime, DateTime inputThrough, DateTime inputThroughTime, bool useDateTimeRange,
            String searchPattern, String replacement, bool searchAll,
            bool searchTrash, bool matchCase, bool matchWholeWord,
            bool replace, bool multiline, bool explicitCaptures,
            bool searchReplaceTitle)
        {
            ...
            // now process all records
            // first get the total number of chapters which exist in db
            long nodesCount = OpenFileSystemDB.NodesCount(ctx);
            if (nodesCount <= 0)
                return false;
            ...
            rtb.SuspendLayout();
            rtb.BeginUpdate();
            List<myNode> nodes = OpenFileSystemDB.findAllNodes(ctx, true, false);
            long nodesDone = 0;

            foreach (myNode listNode in nodes)
            {
                myNode node = listNode;
                String rtf = OpenFileSystemDB.loadNodeData(ctx, node.chapter.guid);
                ...
                if (matchesFound)
                {
                    OpenFileSystemDB.updateNode(ctx, ref node, rtb.Rtf, true);
                    __insertLvSearchItem(lv, node.chapter, totalMatches);
                }
            }
            tsProgressBar.Value = 0;
