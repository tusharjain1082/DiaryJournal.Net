...
//            tsbuttonSave.Checked = true;
        ...
        private void bgWorkerExportEntries_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            ...
            foreach (myTreeDomNode listedNode in tree)
            {
                ...
                entryMethods.exportEntry(cfg, node.chapter.Id, browseFolder.SelectedPath, false, exportIndex, entryType);
        ...
        public void promoteNode()
        {
            ...
            myNode? node = null;
            Int64 id = Int64.Parse(tvEntries.SelectedNode.Name);
            if (!entryMethods.DBPromoteNode(cfg, id, ref node))
                return;
        ...
        public void promoteNodeToRoot()
        {
            ...
            myNode? node = null;
            Int64 id = Int64.Parse(tvEntries.SelectedNode.Name);
            if (!entryMethods.DBPromoteNodeToRoot(cfg, id, ref node))
                return;
        ...
        public void __moveNodeTo(TreeNode? subjectTreeNode)
        {
            ...
            form.allNodes = entryMethods.DBFindAllNodes(cfg);
            form.allNodes = entryMethods.DBFindAllNodesTreeSequence(cfg, ref form.allNodes, true, false);
            if (form.ShowDialog() != DialogResult.OK)
                return;

            TreeNode?  selectedTreeNode = form.tvEntries.SelectedNode;
            if (selectedTreeNode == null) // no node selected, abort.
                return;

            Int64 targetNodeId = Int64.Parse(selectedTreeNode.Name);
            Int64 id = Int64.Parse(subjectTreeNode.Name);

            if (id == targetNodeId)
                return; // same target node selected as the subject node, so abort.

            myNode? node = null;
            if (!entryMethods.DBSetNodeParent(cfg, id, ref node, targetNodeId))
                return;
        ...
        {
            sfdFile.Filter = "*.rtf|*.rtf|*.txt|*.txt|*.html|*.html";
            ...
            // straightaway export the entry
            if (entryMethods.exportEntry(cfg, id, sfdFile.FileName, true, 0, entryType))
