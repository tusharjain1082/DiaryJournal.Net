...
namespace DiaryJournal.Net
{
    public static class entryMethods
    {
            ...
            bool topToBottom = false, bool includeNonDeleted = false)
        {
            ...
            // first obtain all non deleted nodes
            List<myNode> deletedNodes = allNodes.Where(a => a.chapter.IsDeleted == true).ToList();
            // now obtain all deleted nodes
            List<myNode> nonDeletedNodes = allNodes.Where(a => a.chapter.IsDeleted == false).ToList();

            // if user demands to find deleted nodes, then validate and add accordingly
            if (includeNonDeleted)
                worklist.AddRange(nonDeletedNodes);

            if (deleted)
                worklist.AddRange(deletedNodes);
            ...
            while (true)
            {
                ...
                node = FindNodeInList(ref worklist, node.chapter.parentId);
                if (node == null)
