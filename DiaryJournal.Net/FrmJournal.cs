...
//                var res = MessageBox.Show(this, "you cannot directly close this form. please click on exit button to close the form.", "error",
  //                      MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public void setupNewEntry(DateTime dateTime, myNode? parent = null)
        {
            ...
            {
                // create new node
                newNode = OpenFileSystemDB.newNode(ref cfg.ctx1, NodeType.EntryNode, dateTime, parentChapter.guid, true, "");
                if (newNode == null)
                    return;
            ...
            {
                newNode = SingleFileDB.newNode(ref cfg.ctx0, NodeType.EntryNode, dateTime, parentChapter.guid, true, "");
                if (newNode == null)
                    return;
        ...
        public void __importTheJournalRtfEntries(String path)
        {
            ...
            foreach (string file in files)
            {
                ...
                {
                    // import direct
                    if (!OpenFileSystemDB.createNode(cfg.ctx1, ref node, rtf))
                        continue;
                }
                else
                {
                    // import direct
                    if (!SingleFileDB.createNode(cfg.ctx0, ref node, rtf))
                        continue;
                }
        ...
        public void __SingleFileDBToOpenFSDB(String path, String dbName, EntryType entryType)
        {
            ...
            // collect all nodes
            List<myNode> nodes = SingleFileDB.findAllNodes(cfg.ctx0, true, false);
