...
namespace DiaryJournal.Net
{
    public static class xmlEntry
    {
        ...
        public const string entryGuidTagName = "entryGuid";
        public const string entryTextTagName = "entryText";
        public const string entryParentGuidTagName = "entryParentGuid";
        ...
        public static bool fromXml(ref Chapter chapter, String file, ref String rtf)
        {
            ...
            list = entryElement.GetElementsByTagName(entryGuidTagName);
            if (list.Count <= 0)
                return false;

            // we got entry guid child element
            XmlElement child1 = (XmlElement)list[0];
            chapter.guid = Guid.Parse(child1.InnerText);
            ...
            list = entryElement.GetElementsByTagName(entryParentGuidTagName);
            if (list.Count <= 0)
                return false;

            // we got entry parent guid element
            XmlElement child3 = (XmlElement)list[0];
            chapter.parentGuid = Guid.Parse(child3.InnerText);
            ...
            list = entryElement.GetElementsByTagName(entryYearTagName);
            if (list.Count <= 0)
                return false;

            // we got entry is deleted element
            XmlElement child9 = (XmlElement)list[0];
            chapter.year = int.Parse(child9.InnerText);

            list = entryElement.GetElementsByTagName(entryMonthTagName);
            if (list.Count <= 0)
                return false;

            // we got entry is deleted element
            XmlElement child10 = (XmlElement)list[0];
            chapter.month = int.Parse(child10.InnerText);
        ...
        public static String toXml(Chapter chapter, String rtf)
        {
            //xml Decalration:
            XmlDocument doc = new XmlDocument();
            ...
            XmlElement child2 = doc.CreateElement(string.Empty, entryGuidTagName, string.Empty);
            child2.InnerText = chapter.guid.ToString();
            entryElement.AppendChild(child2);
            XmlElement child3 = doc.CreateElement(string.Empty, entryParentGuidTagName, string.Empty);
            child3.InnerText = chapter.parentGuid.ToString();
            ...
            XmlElement child9 = doc.CreateElement(string.Empty, entryYearTagName, string.Empty);
            child9.InnerText = chapter.year.ToString();
            entryElement.AppendChild(child9);
            XmlElement child10 = doc.CreateElement(string.Empty, entryMonthTagName, string.Empty);
            child10.InnerText = chapter.month.ToString();
            entryElement.AppendChild(child10);
