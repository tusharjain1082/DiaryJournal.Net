...
//            this.Invoke(showMessageBox, "total entries imported:" + filesDone, "done", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        public bool exportEntry(Chapter chapter, String path, long exportIndex, EntryType entryType, bool freeStandingEntry = false)
        {
            if (chapter == null)
                return false;
            ...
            if (!freeStandingEntry)
            {
                ...
                file = @"\\?\" + file;
                */
                String file = entryMethods.getFormattedJournalPathFileName(path, chapter.guid, chapter.parentGuid, chapter.Title,
                    chapter.chapterDateTime, exportIndex, entryType);
                File.WriteAllText(file, fileData);
            }
            else
            {
                // exclusive free standing entry. so the path parameter should be the filename and it's location.
                path = @"\\?\" + path;
...
//            tsbuttonSave.Checked = true;
        ...
        {
            exportSelectedFreeStandingEntry(EntryType.Xml);
        }

        public void exportSelectedFreeStandingEntry(EntryType entryType)
        {
            // first get the entry type and formats
            String ext = "";
            String extComplete = "";
            ...
            TreeNode node = tvEntries.SelectedNode;
            ...
            sfdFile.InitialDirectory = Application.StartupPath;
            String entryName = entryMethods.getFormattedJournalFileName(chapter.guid, chapter.parentGuid, chapter.Title,
                chapter.chapterDateTime, 0, entryType);
            sfdFile.FileName = entryName;
            if (sfdFile.ShowDialog() != DialogResult.OK)
                return;

            if (exportEntry(chapter, sfdFile.FileName, 0, entryType, true))
            {

            }
        }

        public void importFreeStandingEntry(EntryType entryType)
        {
            // first get the entry type and formats
            String ext = "";
            String extComplete = "";
            ...
            // matching file found, import direct
            Chapter? chapter = null;
            chapter = entryMethods.importEntry(ctx, ofdFile.FileName, entryType, default(DateTime), true);
            // if error, then skip this file
            if (chapter == null)
                return;

            DateTime customDateTime = chapter.chapterDateTime;
            if (userInterface.ShowDateTimeDialog("new free standing entry's date and time (optional)",
                ref customDateTime, CalendarEntries.BoldedDates) == DialogResult.OK)
            {
                // user selected custom date and time for this free standing entry. update it in db
                chapter.chapterDateTime = customDateTime;
                myDB.updateDBChapter(ctx, ref chapter);
            }
        ...
        {
            importFreeStandingEntry(EntryType.Xml);
        }

        private void toolStripMenuItem27_Click(object sender, EventArgs e)
        {
            exportSelectedFreeStandingEntry(EntryType.Rtf);
        }

        private void toolStripMenuItem28_Click(object sender, EventArgs e)
        {
            importFreeStandingEntry(EntryType.Rtf);
        }
    }
